	...
	<div id="wrapper">
		...
		foreach($showMailBatchInfo_teg_Details as $showMailBatchInfo_teg_Detailskey =>$showMailBatchInfo_teg_DetailsValue){
							...
							// print_r($showMailBatchInfoDetails[$showMailBatchInfoDetailsKey]->tag);
							  if($showMailBatchInfo_teg_Details[$showMailBatchInfo_teg_Detailskey]->tag == 'good' && $showMailBatchInfo_teg_Details[$showMailBatchInfo_teg_Detailskey]->tag != ''){
								  
								  $GoodProcessed++;
								  
							  }
							  if($showMailBatchInfo_teg_Details[$showMailBatchInfo_teg_Detailskey]->tag != 
							 'good'  && $showMailBatchInfo_teg_Details[$showMailBatchInfo_teg_Detailskey]->tag != ''){
			...
			$details='<table class="mailSummary active" border="1">
						...
						<thead class="thead-inverse">
							...
							<tr>
								<td>Un-mailable</td>
								<td id="td_Un_mailable">'.$BedProcessed.'</td>
							</tr>
